...
Named Hermes-authored fragments of the system prompt
carry stable string *keys* (see :data:`FRAGMENT_KEYS`). Users reshape them
...
Design contract — **overrides are pure data, loaded once at construction
and applied at prompt-build time**. There is no callable hook or conditional logic: the
...
this session. ``append``/``prepend`` to a fragment that isn't present this session
...
FRAGMENT_KEYS: Dict[str, str] = {
    ...
    "computer_use": "Retired built-in block; retained key, currently a no-op.",
    "nous_subscription": "Retired built-in block; retained key, currently a no-op.",
