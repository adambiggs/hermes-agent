...
security:
  trusted_private_ip_hosts:
    - review-surface.example.invalid
```

Entries are exact hostnames, matched case-insensitively; subdomains are **not** covered, so listing `example.invalid` does not admit `evil.example.invalid`. Requests to a listed host must use HTTPS — plain `http://` stays blocked. The cloud-metadata floor is still checked first, so a listed host that resolves to `169.254.169.254` is blocked anyway, and DNS failures still fail closed. <!-- snubline-ignore: ip -->
