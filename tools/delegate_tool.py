...
def delegate_task(
    ...
    children = []
    # Resolved model per task, in task order — status metadata for the async
    # delegation record (a fan-out may be heterogeneous under per-task
    # provider routing).
    ...
    try:
        for i, t in enumerate(task_list):
            ...
            task_models.append(creds["model"])
    ...
    if background:
        ...
        # Distinct resolved models in task order — one label when the batch is
        # homogeneous, a joined list when per-task routing mixed endpoints.
        _batch_models = [m for m in dict.fromkeys(task_models) if m]
        dispatch = dispatch_async_delegation_batch(
            ...
            toolsets=None,
            role=top_role,
            model=", ".join(_batch_models) if _batch_models else None,
...
def _build_provider_param_description() -> str:
    ...
    return (
        ...
        + "; ".join(parts)
        + ". Subagents inherit your toolsets; a provider's toolset restriction "
        "(shown above) is enforced on top of that and cannot be widened."
    )
        ...
        "properties": {
            ...
            "provider": {
                ...
                "description": (
                    ...
                    "subagent can be restricted to fewer toolsets than it would "
                    "otherwise inherit from you."
                ),
            },
            "model": {
                "type": "string",
